...
public actor VaelenCoreClient {
    ...
    public func routeObservedList() async throws -> RouteObservedListResult { guard case .routeObservedList(let result) = try result(from: await send(IPCRequest(method: .routeObservedList))) else { throw CoreClientError.invalidResponse }; return result }
    ...
    public func tlsTrustLocalCA() async throws -> TLSTrustResult { guard case .tlsTrust(let result) = try result(from: await send(IPCRequest(method: .tlsTrustLocalCA))) else { throw CoreClientError.invalidResponse }; return result.result }
    public func tlsRemoveLocalCATrust() async throws -> TLSTrustResult { guard case .tlsTrust(let result) = try result(from: await send(IPCRequest(method: .tlsRemoveLocalCATrust))) else { throw CoreClientError.invalidResponse }; return result.result }
