...
public actor DNSResponderSupervisor: DNSResponderControlling {
    ...
    public func start() throws -> DNSResponderIdentity {
        ...
        let identity = DNSResponderIdentity(pid: child.processIdentifier, executablePath: executablePath, arguments: arguments, startedAt: processStartIdentity(child.processIdentifier))
    ...
    public func stop(expected: DNSResponderIdentity, timeout: TimeInterval) throws -> DNSResponderStatus {
        ...
        guard let child = ownedChild, let identity = ownedIdentity, identity == expected, child.processIdentifier == expected.pid,
              child.executablePath == expected.executablePath, child.arguments == expected.arguments else {
            throw DNSCapabilityError.responderUnverified("No retained launch handle proves Vaelen owns responder PID \(expected.pid); no signal was sent")
        }
        if child.isRunningAndReapedIfExited() {
...
public actor DNSCapability {
    ...
    public func status() async -> DNSStatus {
        ...
        if ownershipLoadFailed { return DNSStatus(state: .conflict, ownership: .unknown, port: port, pid: process.pid, responderState: process.state, health: "ownership-uncertain", conflict: "Durable resolver ownership record is unreadable; no resolver change was attempted.") }
        ...
        if let record, record.phase == .resolverRestored {
            ...
            case .unverified: return DNSStatus(state: .conflict, ownership: record.previous.exists ? .external : .none, resolverContent: inspection.content, port: port, pid: process.pid, responderState: process.state, health: "resolver-restored-responder-unverified", conflict: "Resolver state was restored, but the responder cannot be safely signaled without its original process handle.")
            case .exited, .stopped: return DNSStatus(state: .unhealthy, ownership: record.previous.exists ? .external : .none, resolverContent: inspection.content, port: port, pid: process.pid, responderState: process.state, health: "responder-shutdown-pending", conflict: "Resolver is restored; responder cleanup confirmation and durable record removal remain pending.")
            }
        }
        ...
        if inspection.exists && inspection.ownership == .unknown && expectedContent == nil { return DNSStatus(state: .conflict, ownership: .unknown, resolverContent: inspection.content, port: port, pid: process.pid, responderState: process.state, health: "conflict", conflict: "External /etc/resolver/test exists") }
    ...
    public func install(takeover: Bool = false) async throws -> DNSStatus {
        ...
        if let record = existingRecord {
            ...
            guard inspection.snapshot == record.previous else { throw DNSCapabilityError.ownershipMismatch }
            ...
            if inspection.exists && !takeover { throw DNSCapabilityError.externalConflict }
        ...
        let responderIdentity = try await responder.start()
        ...
        do { try ledger?.saveResolverOwnershipRecord(record) }
            ...
            _ = try? await responder.stop(expected: responderIdentity, timeout: responderShutdownTimeout)
            throw DNSCapabilityError.ownershipUncertain("Could not durably record the launched DNS responder identity; resolver file was not changed")
        }
        do {
    ...
    public func remove() async throws -> DNSStatus {
        try await restoreResolverAndStopResponder(retainingAuthorization: false)
    }
    ...
    public func shutdownForQuit() async throws -> DNSStatus {
        try await restoreResolverAndStopResponder(retainingAuthorization: true)
    }
    ...
    private func restoreResolverAndStopResponder(retainingAuthorization: Bool) async throws -> DNSStatus {
        ...
        if record.phase == .resolverRestored {
            let inspection = try await helper.inspectTestResolver()
            if retainingAuthorization, inspection.snapshot != record.previous {
                let process = await responder.status()
                guard process.state == .stopped || process.state == .exited else {
                    throw DNSCapabilityError.responderUnverified("Resolver was changed externally and the DNS responder is not verified stopped")
                }
                return await status()
            }
        ...
        let inspection = try await helper.inspectTestResolver()
        if record.phase == .restoring, inspection.snapshot == record.previous {
        ...
        guard record.phase == .owned || record.phase == .restoring, inspection.snapshot == written else { throw DNSCapabilityError.ownershipMismatch }
        let restoring = DNSResolverOwnershipRecord(phase: .restoring, previous: record.previous, intendedBytes: record.intendedBytes, written: written)
        ...
        do {
            ...
            guard after.snapshot == record.previous else { throw DNSCapabilityError.ownershipUncertain("Resolver restore verification did not match the saved preimage") }
            let restored = DNSResolverOwnershipRecord(phase: .resolverRestored, previous: record.previous, intendedBytes: record.intendedBytes, written: written, responder: record.responder)
            try ledger?.saveResolverOwnershipRecord(restored); resolverRecord = restored
            try await finishResponderCleanup(restored, retainingAuthorization: retainingAuthorization)
        } catch {
    ...
    private func finishResponderCleanup(_ record: DNSResolverOwnershipRecord, retainingAuthorization: Bool) async throws {
        ...
        guard let identity = responderStatus.identity else {
    ...
    private func loadResolverRecord() -> DNSResolverOwnershipRecord? {
        do { if let loaded = try ledger?.resolverOwnershipRecord() { resolverRecord = loaded } }
